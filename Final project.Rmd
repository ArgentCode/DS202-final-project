...
HousingOriginal %>% 
  ...
  theme_bw()
fivenum(HousingOriginal$yearbuilt)
```

2 sentences summary: Our median was 1963 with 75% of the data being between 1950 and 1981. Our data is skewwed left with a logical limit of 2017 when the Kaggle competition was taking place.
...
2 sentences summary: Data is unimodal and bell shaped with a right skew. There is a logical barrier at 0 as you cannot have negative bathrooms. Mean is right around 3 which is where its expected to be.
...
2 sentences summary: As predicted, most of the observations have no fireplace. Results look logarithmic.
...
fivenum(HousingOriginal$basementsqft)
#Show relationships between basementsqft vs taxvaluedollarcnt to see if the expectation match
# To see if they have linear relationship
```

2 sentences summary: The median square feet of a house's basement is 533 with 75% of the basement's area between 266 to 823.5 square feet. The graph skewed right. 
...
fivenum(HousingOriginal$finishedsquarefeet15)
#Show relationships between basementsqft vs taxvaluedollarcnt to see if the expectation match
# To see if they have linear relationship
...
fivenum(HousingOriginal$landtaxvaluedollarcnt)
#Show relationships between basementsqft vs taxvaluedollarcnt to see if the expectation match
# To see if they have linear relationship
```
...
fivenum(HousingOriginal$garagetotalsqft)
#Show relationships between basementsqft vs taxvaluedollarcnt to see if the expectation match
# To see if they have linear relationship
```
...
HousingOriginal %>% 
  ggplot(aes(x=airconditioningtypeid))+
  geom_line(stat="density", color="Red", size=1)+
  theme_bw()
```
...
HousingOriginal %>% 
ggplot(aes(x=typeconstructiontypeid))+geom_line(stat="density", color="Red", size=1)+theme_bw()
fivenum(HousingOriginal$typeconstructiontypeid)
#Show relationships between basementsqft vs taxvaluedollarcnt to see if the expectation match
# To see if they have linear relationship
