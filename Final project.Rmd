...
landtaxvaluedollarcnt: the land tax usually base on the house's price. For example in Iowa, land tax is 1.53%. So higher house's value, the higher tax land. 
...
fivenum(HousingOriginal$bathroomcnt)
...
fivenum(HousingOriginal$fireplacecnt)
```
...
```{r}
# adding relationship between Categorical vs categorical using bar chart
...
# Replaced variables' nas with 0
NgocHousing$basementsqft =   NgocHousing$basementsqft %>% replace_na(0)
NgocHousing$garagetotalsqft =   NgocHousing$garagetotalsqft %>% replace_na(0)
NgocHousing$inishedsquarefeet15 = NgocHousing$finishedsquarefeet15 %>% replace_na(0)
NgocHousing$typeconstructiontypeid =   NgocHousing$typeconstructiontypeid %>% replace_na(0)
